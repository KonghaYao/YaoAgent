...
// type guard
function isDynamicTool(tool: StructuredToolInterface | DynamicTool | RunnableToolLike): tool is DynamicTool {
    return "schema" in tool && "name" in tool && "description" in tool && "responseFormat" in tool;
}

const createHandoffTool = ({ agentName, description, updateState }: CreateHandoffToolParams) => {
    /**
