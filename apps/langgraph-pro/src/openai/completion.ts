...
import { AIMessage, AIMessageChunk, type BaseMessage, isAIMessage, type UsageMetadata, type BaseMessageFields, BaseMessageChunk } from "@langchain/core/messages";
...
import { OpenAIToolChoice, formatToOpenAIToolChoice, _convertToOpenAITool } from "./tools.js";
import { BaseChatOpenAICallOptions } from "@langchain/openai";
import { BaseChatOpenAI } from "@langchain/openai";
import { convertCompletionsDeltaToBaseMessageChunk, convertCompletionsMessageToBaseMessage, convertMessagesToCompletionsMessageParams } from "./converter.js";
 ...
 * @internal
 */
export class ChatOpenAICompletions<CallOptions extends ChatOpenAICompletionsCallOptions = ChatOpenAICompletionsCallOptions> extends BaseChatOpenAI<CallOptions> {
    /** @internal */
    override invocationParams(options?: this["ParsedCallOptions"], extra?: { streaming?: boolean }): ChatCompletionsInvocationParams {
        ...
        const params: Partial<ChatCompletionsInvocationParams> = {
            ...
            tools: options?.tools?.length ? options.tools.map((tool) => this._convertChatOpenAIToolToCompletionsTool(tool, { strict })) : undefined,
            tool_choice: formatToOpenAIToolChoice(options?.tool_choice as OpenAIToolChoice),
    ...
    async _generate(messages: BaseMessage[], options: this["ParsedCallOptions"], runManager?: CallbackManagerForLLMRun): Promise<ChatResult> {
        ...
        const messagesMapped: OpenAIClient.Chat.Completions.ChatCompletionMessageParam[] = convertMessagesToCompletionsMessageParams({
            messages,
            model: this.model,
        });
        ...
        if (params.stream) {
            ...
            for (const part of data?.choices ?? []) {
                ...
                /** @ts-ignore */
                const reasoning_content = part.message?.reasoning_content ?? part.message?.reasoning ?? "";
                ...
                generation.message = new AIMessage(Object.fromEntries(Object.entries(generation.message).filter(([key]) => !key.startsWith("lc_"))) as BaseMessageFields);
    ...
    async completionWithRetry(request: OpenAIClient.Chat.ChatCompletionCreateParamsNonStreaming, requestOptions?: OpenAIClient.RequestOptions): Promise<OpenAIClient.Chat.Completions.ChatCompletion>;
